...
def search(client, user_query, index="bbuy_products", sort="_score", sortDir="desc", use_synonym=False):
    ...
    name_field = 'name.synonym' if use_synonym else 'name'
...
if __name__ == "__main__":
    ...
    general.add_argument('--synonym', default=False, action='store_true', help='Use name.synonym instead of name')
    ...
    if args.synonym:
        logger.info("Synonym will be used")
    ...
    while query != "Exit":
        search(client=opensearch, user_query=query, index=index_name, use_synonym=args.synonym)
        query = input(query_prompt)
