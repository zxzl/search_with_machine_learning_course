...
class DataPrepper:
    ...
    def __log_ltr_query_features(self, query_id, key, query_doc_ids, click_prior_query, no_results, terms_field="_id"):
        ...
        # doc_id -> name_match
        feature_map = {}
        feature_map['name_match'] = {}
        default_value = 0
        for hit in response['hits']['hits']:
            featuremap = { entry['name']: entry.get('value', default_value)
                            for entry in hit['fields']['_ltrlog'][0]['log_entry']}
            feature_map['name_match'][hit['_id']] = featuremap.get('name_match', default_value)
        for doc_id in query_doc_ids:
            feature_results["name_match"].append(feature_map['name_match'].get(doc_id, default_value))
